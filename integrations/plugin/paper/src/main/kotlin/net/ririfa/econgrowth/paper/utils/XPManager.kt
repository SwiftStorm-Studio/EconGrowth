...
import net.ririfa.econgrowth.core.utils.toShortUUID
...
object XPManager {
    ...
    fun generateLevelToXPMap(maxLevel: Int, type: XPType): Map<Int, Float> {
        ...
        var baseXP = if (type == XPType.PROFESSION) 500.0f else 1500.0f
        for (level in 1..maxLevel) {
            levelToXPMap[level] = baseXP
            baseXP *= getGrowthFactor(level, type)
        }
    ...
    fun calculateXPForKillEntity(
        ...
        val playerUUID = player.uniqueId.toShortUUID()
        val killedPlayerUUID = entity.uniqueId.toShortUUID()
